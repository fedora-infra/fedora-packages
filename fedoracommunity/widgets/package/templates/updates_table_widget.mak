    ...
    <table id="${w.id}">
        ...
        <tbody class="rowtemplate">
            <tr class="${'${status}'}-update ${'${type}'}-update">
                ...
                <td>${'${status}'}
                    <div class="karma_${karma_level}"><a href="https://admin.fedoraproject.org/updates/${'${title}'}" moksha_url="static"><img src="${tg.url('/images/16_karma-${karma_level}.png')}" />${'${karma_str}'} karma</a></div>
                </td>
                <td>
                  {{html details}}
                </td>
