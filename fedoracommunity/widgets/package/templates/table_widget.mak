        ...
        <table id="${id}" class="${table_class}">
            ...
            <tbody class="rowtemplate">
                    <tr class="@{state:filter(get_state_class)}">
                        <td class="one-row state"><img src="${tmpl_context.get_url('/images/16_build_state_@{state}.png')}" />&nbsp;@{state_str}</td>
                        <td class="one-row">
...
<h4>Quick Links for <strong>@{package_name}</strong>:</h4>
                            <ul>
                               ...
                               <li><a href="https://admin.fedoraproject.org/pkgdb/package/@{package_name}" moksha_url="dynamic"><img src="${tmpl_context.get_url('/images/16_pkgdb.png')}"/><span>Go to @{package_name} <strong>package info</strong> in PackageDB</span><img src="${tmpl_context.get_url('/images/16_offsite-link.png')}"/></a></li>
                               <li><a href="https://bugzilla.redhat.com/buglist.cgi?query_format=advanced&classification=Fedora&product=Fedora&component=@{package_name}&bug_status=__open__"><img src="${tmpl_context.get_url('/images/16_bugzilla.png')}"/><span>Go to @{package_name} <strong>bugs</strong> in Bugzilla</span><img src="${tmpl_context.get_url('/images/16_offsite-link.png')}"/></a></li>
